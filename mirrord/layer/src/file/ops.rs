use core::ffi::CStr;
use std::{backtrace::Backtrace, ffi::CString, io::SeekFrom, os::unix::io::RawFd, path::PathBuf};
...
pub(crate) fn open(rawish_path: Option<&CStr>, open_options: OpenOptionsInternal) -> Detour<RawFd> {
    ...
    println!("Locking OPEN_FILES 4.");
    OPEN_FILES
        .lock()
        .unwrap()
        .insert(local_file_fd, Arc::new(RemoteFile::new(remote_fd)));
    println!("Freeing OPEN_FILES 4.");
...
pub(crate) fn fdopendir(fd: RawFd) -> Detour<usize> {
    ...
    println!("Locking OPEN_FILES 7.");
    let remote_file_fd = OPEN_FILES
        .lock()?
        .get(&fd)
        .ok_or(Bypass::LocalFdNotFound(fd))?
        .fd;
    println!("Freeing OPEN_FILES 7.");
    ...
    println!("Locking OPEN_FILES 8.");
    OPEN_FILES.lock()?.remove(&fd);
    println!("Freeing OPEN_FILES 8.");
...
pub(crate) fn openat(
    ...
    if path.is_absolute() || fd == AT_FDCWD {
        ...
        println!("Locking OPEN_FILES 9.");
        OPEN_FILES
            .lock()?
            .insert(local_file_fd, Arc::new(RemoteFile::new(remote_fd)));
        println!("Freeing OPEN_FILES 9.");
